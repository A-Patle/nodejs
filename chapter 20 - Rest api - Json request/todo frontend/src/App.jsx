...
function App() {
  ...
  useEffect(() => {
    getItemToServer().then((initialItem) => {
      setTodoItems(initialItem);
    });
  }, []);
  ...
  const handleDeleteItem = async (id) => {
    const deletedId = await deleteItemOnServer(id);
    const newTodoItems = todoItems.filter((item) => item.id !== deletedId);
    setTodoItems(newTodoItems);
  };

  const handleToggleComplete = async (id) => {
    await markCompletedOnServer(id);
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden mb-8">
          <div className="p-8">
            ...
            <TodoItems
              ...
              onToggleComplete={handleToggleComplete}></TodoItems>
          </div>
        </div>
      </div>
    </div>
